...
def getind(mt,nt):
	full_idxt=mt*[range(nt)]+ nt*np.arange(mt)[:,None]
	
	# shift by +1
	idxtf=mt*[np.concatenate(([nt-1],range(nt-1)),axis=0)]
	full_idxtf = idxtf + nt*np.arange(mt)[:,None]
	
	# shift by -1
	idxtb=mt*[np.concatenate((range(1,nt),[0]),axis=0)]
	full_idxtb = idxtb + nt*np.arange(mt)[:,None]
	return full_idxtf,full_idxtb,full_idxt
	
	
def getindn(full_idxtf,full_idxtb,full_idxt,n):
	# index for t
	mt,nt=full_idxtf.shape
	full_idxtff=np.vstack((full_idxtf[n:mt+1],full_idxtf[0:n]))
	full_idxtbf=np.vstack((full_idxtb[n:mt+1],full_idxtb[0:n]))
	full_idxtf=np.vstack((full_idxt[n:mt+1],full_idxt[0:n]))
	
	return full_idxtff,full_idxtbf,full_idxtf
	

def interpolate_fn(t, f, n,full_idxtf1,full_idxtb1,full_idxt1, interpolate=True,getindex=False):
    if (interpolate):
	if (getindex):
		full_idxtfe=full_idxtf1[0:2]
		full_idxtbe=full_idxtb1[0:2]
		full_idxte=full_idxt1[0:2]
		full_idxtf,full_idxtb,full_idxt=getindn(full_idxtfe,full_idxtbe,full_idxte,n)
	else:
		full_idxtf,full_idxtb,full_idxt=getindn(full_idxtf1,full_idxtb1,full_idxt1,n)
	#print(full_idxtf)
	t0 = np.take(t,full_idxtf)
	t1 = np.take(t,full_idxt)
	t2 = np.take(t,full_idxtb)
	f0 = np.take(f,full_idxtf)
	f1 = np.take(f,full_idxt)
	f2 = np.take(f,full_idxtb)
	
#	print(sum(sum(t0-np.roll(t,(-n,1),axis=(0,1)))))
#	print(sum(sum(t1-np.roll(t,(-n,0),axis=(0,1)))))
#	print(sum(sum(t2-np.roll(t,(-n,-1),axis=(0,1)))))
#	print(sum(sum(f0-np.roll(f,(-n,1),axis=(0,1)))))
#	print(sum(sum(f1-np.roll(f,(-n,0),axis=(0,1)))))
#	print(sum(sum(f2-np.roll(f,(-n,-1),axis=(0,1)))))
...
def delta_f(f, t,full_idxtf1,full_idxtb1,full_idxt1):
    #in ring interior
    f_plus  = interpolate_fn(t, f,  1,full_idxtf1,full_idxtb1,full_idxt1)
    f_minus = interpolate_fn(t, f, -1,full_idxtf1,full_idxtb1,full_idxt1)
    if (f.shape[0] > 2):
        df = (f_plus - f_minus)/2
    else:
        df = np.zeros_like(f)
...
def ring_gravity(lambda0, G_ring, r, t,full_idxtf1,full_idxtb1,full_idxt1):
    two_G_lambda = 2.0*G_ring*lambda0
    Ar = np.zeros_like(r)
    Nr, Nt = r.shape
    
    for shft in range(1, Nr):
        dr = interpolate_fn(t, r, -shft,full_idxtf1,full_idxtb1,full_idxt1) - r
        Ar += two_G_lambda/dr
    return Ar

#acceleration due to pressure P
def A_P(lambda0, sd, P, t, delta_P, delta_r,full_idxtf1,full_idxtb1,full_idxt1):
    ...
    P_outer = interpolate_fn(t[-2:], P[-2:], 1,full_idxtf1,full_idxtb1,full_idxt1,getindex=True)[-1]
    A[-1] = P_outer/lambda0[-1]
    return A

#radial acceleration due to ring pressure
def ring_pressure(c, lambda0, sd, r, t, delta_r,full_idxtf1,full_idxtb1,full_idxt1):
    P = (c*c)*sd
    delta_P = delta_f(P, t,full_idxtf1,full_idxtb1,full_idxt1)
    Ar = A_P(lambda0, sd, P, t, delta_P, delta_r,full_idxtf1,full_idxtb1,full_idxt1)
    return Ar

#tangential acceleration due to ring viscosity
def ring_viscosity(shear_viscosity, lambda0, sd, r, t, vt, delta_r,full_idxtf1,full_idxtb1,full_idxt1):
    w = vt/r
    delta_w = delta_f(w, t,full_idxtf1,full_idxtb1,full_idxt1)
    dw_dr = df_dr(delta_w, delta_r)
    #viscous pseudo-pressure
    P = (-shear_viscosity*sd)*r*dw_dr
    delta_P = delta_f(P, t,full_idxtf1,full_idxtb1,full_idxt1)
    At = A_P(lambda0, sd, P, t, delta_P, delta_r,full_idxtf1,full_idxtb1,full_idxt1)
    return At

#calculate radial and tangential accelerations due to ring gravity, pressure, visocisty
def accelerations(lambda0, G_ring, shear_viscosity, c, r, t, vt,full_idxtf1,full_idxtb1,full_idxt1):
    ...
    if (G_ring > 0.0):
        Ar += ring_gravity(lw, G_ring, rw, tw,full_idxtf1,full_idxtb1,full_idxt1)
    #radial acceleration due to streamline pressure and viscosity
    if ((c > 0.0) or (shear_viscosity > 0.0)):
        delta_rw = delta_f(rw, tw,full_idxtf1,full_idxtb1,full_idxt1)
        sdw = surface_density(lw, delta_rw)
        if (c > 0.0):
            Ar += ring_pressure(c, lw, sdw, rw, tw, delta_rw,full_idxtf1,full_idxtb1,full_idxt1)
        if (shear_viscosity > 0.0):
            vtw = wrap_ring(vt, longitude=False)
            At += ring_viscosity(shear_viscosity, lw, sdw, rw, tw, vtw, delta_rw,full_idxtf1,full_idxtb1,full_idxt1)
...
def kick(J2, Rp, lambda0, G_ring, shear_viscosity, c, r, t, vr, vt, dt,full_idxtf1,full_idxtb1,full_idxt1):
    #radial acceleration due to ring gravity and pressure
    Ar, At = accelerations(lambda0, G_ring, shear_viscosity, c, r, t, vt,full_idxtf1,full_idxtb1,full_idxt1)
    #kick velocity
    vr += Ar*dt
    vt += At*dt
    return vr, vt
...
def initialize_streamline(number_of_streamlines, particles_per_streamline, radial_width,
    total_ring_mass, G_ring, Q_ring, shear_viscosity, J2, Rp, initial_orbits,full_idxtf1,full_idxtb1,full_idxt1):  
    ...
    c = 0.0
    if (Q_ring > 0.0):
        delta_r = delta_f(r, t,full_idxtf1,full_idxtb1,full_idxt1)
