...
$\varphi=\theta-\tilde{\omega}$ is the true anomaly {\it i.e.}\ the longitude relative to periapse,
...
for nine ringlets; see quantities $q_{em}$ and $q_{\tilde{\omega}m}$ where $m=1$
...
here\footnote{The exception is Uranus' $\beta$ ringlet whose $q\simeq1.2$ (F24) is so large as to imply that
this ringlet is very disturbed, its steamlines are crossing (\citealt{BGT82}),
and its ring particles are crashing into each other.}. 
