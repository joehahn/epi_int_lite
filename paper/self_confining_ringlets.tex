...
so dividing $t$ by $2\pi$ and then multiplying
by $T_{\text{orb}}$ converts simulation time from natural to physical units.
...
\begin{figure}
    ...
    \caption{
        ...
        (green curve) versus time $t/\tau_\nu$. Dotted red curve has $e'=\sqrt{3}/2\simeq0.866$
        and is the threshold for self-confinement in a non-gravitating ringlet.
    }
\end{figure}\vfil

\pagebreak
\citet{BGT82} examine angular momentum transport in a viscous eccentric non-gravitating ringlet, 
and show that that transport has three noteworthy regimes that are distinguished by the ringlet's $q$:

\begin{enumerate}

\item $q<3/4$. In this case the ringlet's viscous angular momentum flux $F_\nu(\theta)>0$
at all ringlet longitudes $\theta$. $F_\nu$ integrated over all ringlet longitudes
is the ringlet's viscous angular momentum luminosity,
$L_\nu=\oint F_\nu(\theta)d\ell>0$, and $L_\nu>0$ when $q<0.75$.
Viscous friction transports angular momentum radially outwards,
...
\item $3/4<q<\sqrt{3}/4$. In this regime there is a range of longitudes $\theta$
where the viscous angular momentum flux is reversed such that $F_\nu(\theta)<0$. 
Nonetheless $L_\nu$, which is proportional to the orbit-average of $F_\nu(\theta)$,
is positive and the ringlet spreads radially, albeit slower than when $q<0.75$.

\item $q\ge\sqrt{3}/4$. Viscous angular momentum flux reversal is complete, $L_\nu<0$,
...
The viscous flux of angular momentum is (Pringle?)
\begin{equation}
    \label{eqn:F_nu_theta}
    F_{\nu}(r, \theta) = -\nu_s \sigma r^2\frac{\partial\omega}{\partial r}
\end{equation}
where $\omega=\dot{\theta}$ is the angular velocity, Eqn.\ (XX). 
...
derive Eqn.\ \ref{eqn:F_nu_varphi} from \ref{eqn:F_nu_theta}. 

For circular orbits
\begin{equation}
    \label{eqn:F_nu_circular}
    F_{\nu,0}=\frac{3}{2}\nu_s \sigma_0a\Omega
\end{equation}
