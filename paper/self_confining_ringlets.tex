...
If these ringlets are truly self-confining, then they are extremely young, only $\sim{\cal O}(1000)$ years 
...
within a nearby dense planetary ring, presumably as an embedded moonlet aka propeller (\cite{Tetal10}). 
...
The radial migration rate due to the ring-shepherding torque
...
sharp-edged like most observed narrow eccentric ringlets. Nonetheless, self-confinement is temporary
...
until it settles temporarily into the self-confining state, which is also sharp-edged. 
...
only $\sim{\cal O}(1000)$ years. This is due to the ringlet's viscosity, which damps its 
...
again transmit a radial flux of angular momentum that is outwards, so this circularizing ringlet would 
again resume spreading radially, and it would lose its sharp edges.
...
Possibly as a small embedded moonlet like those that are the presumed source of the propeller objects
seen in Saturn's main A ring. If that embedded moonlet is orbiting near the dense ring's edge, then
its reaction to the gravitational shepherding torque that the moonlet exerts on the ring would
