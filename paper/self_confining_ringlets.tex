...
and crosses and stars show those ringlets that did not.
...
\begin{figure}
    ...
    \caption{
        ...
        simulation ID is indicted in Fig.\ \ref{fig:sim_grid_da}. All quantities are plotted 
...
\end{equation}
where $m_r$, $\nu_s$, $\Delta a$ are the simulated ringlet's physical properties, 
...
The exponents in Eqn.\ (\ref{eqn:tau-dyn}) are $\alpha=1.0$, $\beta=-1.0$, $\gamma=0.0$,
and are chosen so that the $q$ versus $t/\tau_{dyn}$ curve for each simulation
in Fig.\ \ref{fig:low_libration_sims} overlaps as much as possible, as
seen in Fig.\ (\ref{fig:q_vs_ID}). All simulations shown in 
Figs.\ \ref{fig:sim_grid_da}--\ref{fig:q_vs_ID}
are evolved for the greater of $10\tau_{dyn}$ or $10\tau_\nu$.
\begin{figure}
