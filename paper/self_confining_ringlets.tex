...
at times $t\gg35\tau_\nu$, its positive viscous angular momentum luminosity $L_\nu\simeq0.0085L_{\nu,0}$ 
...
the ringlet's shape to deviate slightly from the non-gravitating solution of \citet{BGT82}. Interestingly, 
Fig.\ \ref{fig:angular_momentum_luminosity_zoom_nominal} also shows that $L_\nu+L_g$ does not sum
precisely to zero, {\it i.e.}\ $L_L=L_\nu+L_g\simeq0.001L_{\nu,0}$, yet Section \ref{subsec:gravitational_flux}
will show that this ringlet's energy luminosity $L_E$ is zero. Evidently it is 
$L_E$ that must be zero (rather than $L_L$) in order for a viscous gravitating ringlet
to be self-confining, since the streamlines' semimajor axes $a$ are not evolving relative to each other.
That this ringlet's $L_L$ is slightly nonzero while $L_E$ is also implies that this ringlet's
shape {\it i.e.} the its eccentricities are still slowly evolving despite the 
...
\begin{figure}
    ...
    \caption{
        ...
        longitude $\varphi$ at selected moments in time. When the simulation starts, this nearly circular
...
\begin{figure}
    ...
    \caption{
        ...
        with inset plot showing that $\Delta e$ continues to slowly grow despite self-confinement. 
    }
\end{figure}
...
shows how viscous friction tends to transport angular momentum radially inwards, $F_{\nu}(\varphi)<0$, 
at longitudes nearer periapse where $|\varphi|\sim0$ (due to reversal of angular velocity gradient?), and outwards
at all other longitudes. That figure also shows that the ringlet's gravitational
transport of angular momentum is inwards as
ring-matter approaches periapse where $\varphi<0$, 
and is outwards $F_{g}(\varphi)>0$ post-periapse, with this asymmetry being due to the ringlet's
...
The ringlet's total gravitational angular momentum luminosity is computed via
\begin{equation}
    \label{eqn:L_g}
