...
is widely used in theoretical studies of planetary rings c.f.\ \citep{GT79, BGT83, BGT85}
and more recently in N-body studies of rings \citep{HS13, RHH16}. The streamline concept
also allows one to swiftly track the global evolution of the ringlet's streamlines numerically
...
The hydrodynamic approximation is used to handle
...
this approximation accounts for collisional effects
without monitoring individual particle-particle interactions.
The particle's acceleration due to the ringlet's viscosity is
\begin{equation}
\label{eqn:shear_viscosity}
    A_{\nu}=-\frac{1}{\sigma r}\frac{\partial {\cal F}_L}{\partial r},
\end{equation}
where $r$ is the particle's radial coordinate, $\sigma$ is the surface density of ringlet matter,
with acceleration $A_{\nu}$ parallel to the perturbed particle's velocity vector
$\mathbf{v}=\dot{\mathbf{r}}$ where $\mathbf{r} = r\hat{\mathbf{r}}$ is the particle's position vector.
and ${\cal F}_{L,\nu}$ is the flux of angular momentum
that is transported radially across the particle's streamline
due to its collisions with particles in adjacent streamlines:
\begin{equation}
    \label{eqn:F_nu_theta}
    {\cal F}_{L,\nu} = -\nu_s \sigma r^2\frac{\partial\omega}{\partial r}
\end{equation}
where $\nu_s$ is the ringlet's kinematic shear viscosity
and $\omega$ is the particle's angular velocity \citep{HS13}.

Collisions among ring particles also transmits linear momentum in the radial direction,
resulting in a radial acceleration
\begin{equation}
\label{eqn:bulk_viscosity}
    A_{\nu, r}=-\frac{1}{\sigma}\frac{\partial {\cal G}}{\partial r} 
\end{equation}
