...
that ringlet precursor was originally an especially large ring particle that was orbiting within
a dense planetary ring, presumably as an embedded moonlet (reference?). If that embedded moonlet happened to 
form at the edge of the dense planetary ring, then the moonlet's reaction to
the shepherding torque that it exerts on the ring [the radial integral of Eqn.\ (79) of \cite{GT82}] would
cause that moonlet to migrate towards and then beyond the ring's edge. That small moonlet is also
susceptible to collisional disruption by an interplanetary impacter (reference?), and if so then
differential orbital motion would have quickly sheared that debris into a narrow planetary ringlet.
...
as its self-gravity pumps up its eccentricity gradient (Fig. \ref{fig:de_prime_nominal}), 
which if sufficiently vigorous, could cause it to settle into the self-confining state, 
which is sharp-edged like most observed narrow eccentric ringlets. Nonetheless, self-confinement is temporary
and lasts only $\sim{\cal O}(1000)$ years. 
...
until it settles into the sharp-edged self-confining state. Which might only be temporary
because ringlet viscosity can damps the ringlet's eccentricity until the inner edge gets circularized,
which may cause the ringlet's $q$ to drop below the self-confinement threshold, $q\simeq\sqrt{3}/2$.
