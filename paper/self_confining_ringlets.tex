...
such that these ringlets are composed of $N_s=2$ streamlines having $N_p=241$ particles per streamline, initial 
eccentricity $e=0.01$, initial eccentricity gradient $e'=0$, and viscosity $\nu_b=\nu_s$. 
...
where colored squares indicate which ringlets did evolve into a self-confining state,
...
\begin{figure}
    ...
    \caption{
        ...
        Results from a survey of 1155 ringlet simulations having a variety of ringlet
        masses $m_r$, shear viscosities $\nu_s$, with each panel showing outcomes
        for ringlet's having the same initial radial width,
        $\Delta a=0.0003, 0.0005, 0.0008, 0.0012$, or  $0.0016$.
    }
\end{figure}
...
settle into self-confinement with low libration amplititudes, and these ringlets
...
while orange squares indicate those self-confining ringlets that appear most disturbed,
with $\Delta q > 10^{-2}$.

Simulations indicated by a red star never achieve a high enough eccentricity gradient to self-confine,
and their edges will simply spread viscously as $\Delta a(t)\propto t^{1/2}$. 

Simulations indicated by a $\times$ symbol in Fig.\ \ref{fig:sim_grid_da} 
were terminated early because an epi\_int\_lite particle crossed a 
neighboring streamline. In reality, pressure forces would have developed as adjacent streamlines converged,
where particle densities would be very enhanced, particle collisions frequent, with ring particles
possibly rebounding off a shocked region in the ringlet and/or splashing vertically,
none of which is accounted for with this  version of epi\_int\_lite. 
So this survey simply terminates all such simulations and flags that occurrence with a 
...
and possibly nonlinear treatment of pressure effects will need to be implemented in order
