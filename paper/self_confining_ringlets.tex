...
total masses that are geometrically distributed between $2\times10^{-10}\le m_r\le2\times10^{-8}$,
shear viscosities geometrically distributed 
between $3.2\times10^{-13}\le \nu_s\le3.2\times10^{-10}$,
...
where colored squares indicate those ringlets did evolve into a self-confining state,
and crosses and stars for those ringlets that did not.
...
\begin{figure}
    ...
    \caption{
        ...
        1155 ringlet simulations having a variety of ringlet
        masses $m_r$, shear viscosities $\nu_s$, with each panel showing outcomes
        ...
        amplitudes $\Delta q$, red stars for ringlets that fail to settle into self-confinement, 
        and $\times$ for ringlets that terminate early when an epi\_int\_lite particle crossed a 
        neighboring streamline. Numbers indicate the IDs of selected simulations whose time-evolution
        are shown in Fig.\ (\ref{fig:low_libration_sims}), and the nominal ringlet
        simulation has ID=240.
    }
\end{figure}

Five panels are shown in Fig.\ \ref{fig:sim_grid_da},  one for each value of initial $\Delta a$,
and the colored squares there show that there is a single 
...
that settle into self-confinement with low libration amplititudes, and these ringlets
have a nonlinear parameter $q$ that varies by no more than $\Delta q \le 10^{-3}$ 
as the ringlet librates during the simulation's final 20\%.
Green squares indicated those ringlets that are librating with higher amplitudes, 
$10^{-3} < \Delta q \le 10^{-2}$, after settled into self-confinement, 
while orange squares indicate those self-confining ringlets that are most disturbed,
with $\Delta q > 10^{-2}$.

Red stars indicate those simulations that never achieve a high enough eccentricity gradient
to self-confine, and their edges simply spread viscously as $\Delta a(t)\propto t^{1/2}$. 
...
the streamlines in these ringlets will evolve so close to each other that a more sophisticated
and possibly nonlinear treatment of pressure effects would be need to be implemented in order
to accurately assess these ringlets' fates.

Numbers in Fig.\ \ref{fig:sim_grid_da} are the IDs of selected 
ringlet simulations that do settle into self-confinement with modest libration
amplitude $\Delta q$, and the time-histories of those ringlets are
...
mass $m_r$ and viscosity $\nu_s$. The simulations shown in Fig.\ \ref{fig:sim_grid_da}
are admittedly cherry-picked, and are selected to show outcomes for a broad 
range of ringlet parameters $\Delta a, m_r, \nu_s$ that do settle into 
low-libration endstates.
...
\begin{figure}
    ...
    \caption{
        ...
        Each row of plots shows ringlets' nonlinear parameter $q$, semimajor axis width
        $\Delta a$, and streamline eccentricities $e$ over time $t$
        for ringlets having the same or similar mass $m_r$ and viscosity $\nu_s$, for each
        simulation ID'ed by number in Fig.\ \ref{fig:sim_grid_da}.
    }
\end{figure}

The quantities shown in Fig.\ (\ref{fig:low_libration_sims}) are plotted versus $t/\tau_{dyn}$
where the simulated ringlet's dynamical time $\tau_{dyn}$ is

\begin{equation}
    \label{eqn:tau_dyn}
    \tau_{dyn} = \tau_{\nu, n}\left(\frac{m_r}{m_{r,n}}\right)^\alpha
\end{equation}
where $\Delta a$, $m_r$, $\nu_s$ are the properties of the simulated ringlet

\subsection{variations with eccentricity $e$}
