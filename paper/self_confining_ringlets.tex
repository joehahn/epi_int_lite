...
as the ringlet librate about equilibrium during the simulation's final 20\%.
...
Pink diamonds indicate simulations that are ``partially'' confined, these ringlet
do achieve a high $q\sim0.9$, but angular momentum flux reversal is not complete
and so slow viscous spreading is only slowed and not stalled, which 
...
and possibly nonlinear treatment of pressure effects would be needed 
to accurately assess these ringlets' fates.

Numbers in Fig.\ \ref{fig:sim_grid_da} are the IDs of a selection of 
ringlet simulations settle into full or partial self-confinement, 
...
indicated in Fig.\ \ref{fig:sim_grid_da} by pink squares, that row
...
also spread albeit more slowly. So it is probably safe to say that the
...
\begin{figure}
    ...
    \caption{
        ...
        simulation whose ID is also indicted in Fig.\ \ref{fig:sim_grid_da}. All quantities are plotted 
