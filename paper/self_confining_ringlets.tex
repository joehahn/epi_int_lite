...
$\Delta e^0=e_\text{outer} - e_\text{inner}$ such that the entire ringlet will precess as rigid body with its streamlines
having zero relative motions. And when their equation for $\Delta e^0$ is multiplied by $a/\Delta a$
we then have the ringlet's equilibrium dimensionless eccentricity gradient,
\begin{equation}
    \label{eqn:e_prime_eq}
    e'_{eq} = a\frac{\Delta e^0}{\Delta a} = \frac{21\pi}{4}\frac{eJ_2}{m_r/M}\left(\frac{R_p}{a}\right)^2
        \left(\frac{\Delta a}{a}\right)^2\frac{1}{H(q)}
\end{equation}
where the nonlinearity parameter $q=|e'|$ since $\tilde{\omega}'=0$ when an inviscid ringlet is in equilibrium, and the function
\begin{equation}
    \label{eqn:H(q)}
...
a ringlet's equilibrium $e'_{eq}$ numerically via a handful of quick epi\_int\_lite simulations, the results of which are 
...
having small equilibrium $e'_{eq}$.
