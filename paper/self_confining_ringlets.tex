...
Figure \ref{fig:viscous_spreading} examines the radial spreading of a narrow viscous non-gravitating ringlet.
The simulated ringlet has the same physical properties as the nominal ringlet except that it is instead circular and has 
many more streamlines than the $N_s=2$ simulation of Section \ref{subsec:nominal}.
Each colored curve indicates the ringlet's radial surface density profile $\sigma(r)$ at 
various times $t$. Note the the simulated surface density profiles (dashed curves with dots)
track nicely with the theoretical prediction (solid curves) of \cite{P81}. Which is a good indicator that
epi\_int\_lite is calculating a ringlet's viscous evolution correctly.
...
\begin{figure}
    ...
    \caption{
        ...
        Radial spreading of a narrow circular viscous non-gravitating ringlet, simulation (dashed curves with dots)
        versus theory (solid curves).
        ...
        is always axisymmetric only $N_p=3$ particles per streamline are used here. 
        ...
        Solid theoretical curves are from Eqn.\ (2.13) of \cite{P81} and the constant $\bar{\sigma}=m_r/\pi r_{mid}^2$.
    }
\end{figure}

Figure \ref{fig:equilibrium} compares simulation against theory for gravitating ringlet.\\

\begin{figure}
    \plotone{figs/equilibrium.png}
    \caption{
        \label{fig:equilibrium}
        equilibrium $e'$ for inviscid gravitating ringlet, simulation versus theory.
    }
\end{figure}

Figure \ref{fig:libration} compares simulation against theory for gravitating ringlet.\\

\begin{figure}
    \plotone{figs/libration.png}
    \caption{
        \label{fig:libration}
        Libration frequency for inviscid gravitating ringlet, simulation versus theory.
    }
\end{figure}
