...
(Borderies, Shu), whereas for some the execution of an epi\_int\_lite simulation 
might be the simpler/faster way forward. Spiral density waves are nonlinear
when the surface density variation due to the wave is comparable to the ring's undisturbed surface density
{\it i.e.}\ $|\Delta\sigma|\sim\sigma_0$. Which means that, since $\sigma=\lambda/\Delta r$, 
then streamline separations $\Delta r$ need only shrink by a factor of two to make a density wave nonlinear.
Eccentricities associated with the wave are still small, 
...
shoulder-to-shoulder such that further compaction is impossible, 
...
from these traveling incompressbile patches that are sustained by the wave. 
Of particular interest is in Maxwell ringlet...

\item NL spiral bending waves

\item gap embedded moonlets

\item exo-asteroid belts perturbed by exoplanets, and lopsided circumstellar debris disks

All simulations described here use the epi\_int\_lite streamline integrator
