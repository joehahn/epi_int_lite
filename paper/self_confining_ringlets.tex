...
The equation of motion for an unperturbed particle in orbit about the central planet is 
\begin{equation}
    \label{eqn:EOM}
    \ddot{\mathbf{r}}=-\nabla\Phi
\end{equation}
...
\begin{equation}
    ...
    \dot{\theta} = \frac{h}{r^2},
\end{equation}
and inserting the above into the radial part of Eqn.\ (\ref{eqn:EOM}) yields
\begin{equation}
    \label{eqn:radial_EOM}
    \ddot{r} = \frac{h^2}{r^3} -\frac{GM}{r^2} - \frac{3J_2GMR^2}{2r^4}.
\end{equation}
Solving the above to first-order accuracy ordinarily yields
\begin{eqnarray}
...
\end{eqnarray}
with $\Omega$ being the particle's mean orbital frequency and $\kappa$ its epicyclic
frequency. In the above,  $a$, $e$, $\tilde{\omega}$ and $M=\kappa t$ are the epicyclic orbit elements
...
Using the above it is then straightforward to complete the revisions to Eqn.\ (\ref{eqn:linearized_soln}):
\begin{eqnarray}
...
\end{eqnarray}
where $\Omega$ and $\kappa$ are Eqns.\ (\ref{eqn:omega_kappa}) and $M=\kappa t$.
...
\item Calculate $\Omega$ and $\kappa$ using Eqns.\ (\ref{eqn:omega_kappa})

\item Note that $e\sin M = (v_r/a\kappa)(a/r)^3$ and $e\cos M = [(a/r)^2 - 1]/2$, which 
provides the particle's eccentricity via $e = \sqrt{(e\sin M)^2 + (e\cos M)^2}$
and its mean anomaly $M$ from $\tan M = e\sin M/e\cos M$.
