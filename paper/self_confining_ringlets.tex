...
(Borderies, Shu), whereas some might regard the execution of an epi\_int\_lite simulation 
...
nonlinear density waves, which we have confirmed via unpublished experiments. French paper?

\item {\it Incompressible nonlinear spiral density waves}: one can imagine that a sufficiently vigorous nonlinear
...
and that region becomes incompressible. We suspect that these incompressible
...
lead to new and possibly observable phenomena. Note that the equation of state (EOS) employed by
...
from these traveling incompressbile patches that are generated by the wave. 
Of particular interest is in Maxwell ringlet...

\item {\it Nonlinear spiral bending waves}: although theories for nonlinear spiral density waves exist, an
analytic theory for a nonlinear spiral bending waves does not. The fundamental assumption
of linearized bending wave theory is that the radial forces associated with the wave are negligible
compared to the wave's vertical forcing of the ring. Which is true provided the bending wavelength
$\lambda_b$ is always large compared to bending wave amplitude $A$, which is the ring plane's maximal
vertical displacement due to the bending wave, {\it i.e.}\ $\lambda_b\gg A$.  
Although the epi\_int\_lite streamline integrator is 
currently a two dimensional code, adapting it to the 3rd dimension should be straightforward,
which means that code would be well positioned to simulate nonlinear spiral bending waves having
$\lambda_b\lesssim A$.

\item gap embedded moonlets

\item exo-asteroid belts perturbed by exoplanets, and lopsided circumstellar debris disks
