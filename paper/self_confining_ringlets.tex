...
\end{equation}
to lowest order in the ringlet's small eccentricity $e$, noting that $E_{sg}$ is contant when the
ringlet is self-confining.
...
\end{equation}
where $GM\simeq a^3\Omega^2$ to lowest order in $J_2$.
...
The rate at which that patch dissipates orbital energy is $d\dot{E_r}=\delta dA$, so
...
unity except when $e'$ is close to 1, and numerical evaluation shows that $I(e')\simeq0.88$
when $e'=\sqrt{3}/2$.
...
Viscosity circularizes the ringlet in time $\tau_e$, during which time the ringlet's
semimajor axis will have shrunk by $\Delta a=\dot{a}\tau_e=-e_0^2a$ by Eqns. (\ref{eqn:de2/dt})
...
viscous spreading will proceed.

If however a ringlet forms just exterior to an $m=1$ secular resonance, then it is conceivable
that that ringlet might drift into that resonance and become resonantly trapped there, with the resonance
resupplying the ringlet with the orbital energy and eccentricity that would be
needed for long term orbital stability. Or maybe $m=1$ resonance slowly sweep outward and trap...
stability. Or maybe resonance sweep outwards and ...
