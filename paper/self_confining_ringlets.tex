...
({\it ii}.) epi\_int\_lite uses a more accurate drift step to handle
unperturbed motion around an oblate planet (detailed in Appendix \ref{sec:Appendix A}),
and ({\it iii}.) epi\_int\_lite makes the $C=0$ approximation that is justified below 
...
The epi\_int integrator advances the motions of $N$ interacting particles as they orbit 
the primary mass $M$ via an approximate solution to Hamilton's equations of motion.
That solution is derived in \cite{DLL98} and \cite{C99} and summarized again in
Section 2.1 of \cite{HS13}. The Hamiltonian $H$ used there
is a function of what \cite{DLL98} refer to as {\it democratic} and \cite{C99} refer to as
{\it mixed-center} coordinates, with the spatial coordinates of all orbiting bodies
$\mathbf{r}_i$ measured relative to center of primary mass $M$,
and all velocities $\mathbf{v}_i$ measured relative to the system's barycenter,
which is displaced from the primary's center by $\mathbf{r}_0=-\sum_{j=1}^N(m_j/M)\mathbf{r}_j$.

In the mixed-center coordinate system, the Hamiltonian has three parts, $H=H_A + H_B + H_C$,
where $H_A$ accounts for the two-body gravitational forces
exerted between primary mass $M$ and the $N$ other orbiting bodies, $H_B$ accounts for the mutual
interactions exerted between the $N$ small bodies of mass $\mathbf{r}_i$, and $H_C$
is due to the non-inertial movement of the spatial coordinates' origin about the barycenter.
