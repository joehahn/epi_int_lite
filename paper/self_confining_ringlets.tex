...
\subsection{variations with $m_r$, $\nu_s$, and $\Delta a$}
\label{subsec:variations}

To assess whether the nominal ringlet's evolution is typical of other ringlets
having alternate values of total masses $m_r$, shear viscosities $\nu_s$, and initial widths $\Delta a$,
...
and initial radial widths that are linearly distributed between  
$0.0003\le \Delta a\le0.0016$. Survey results are summarized in Fig.\ \ref{fig:sim_grid_da}
where the square symbols indicate those ringlets that did evolve into a self-confining state, 
red asterix to indicate those ringlets that never evolved into the
a self-confining state, and $\times$ for those ringlet simulations that terminated early
because their streamlines crossed. 

Figure \ref{fig:sim_grid_da} has five panels, 
one for each value of initial $\Delta a$, and those panels indicate that there is a single 
island in this three-dimensional ($m_r, \nu_s, \Delta a$) parameter space where survey
simulations do evolve into the self-confining state, with those simulations indicated with
the blue, green, and orange squares. Blue squares indicate those ringlets that have
the lowest libration amplitudes after achieving self-confinement, and these ringlets
have a nonlinear parameter $q$ that varies by no more than $\Delta q \le 10^{-3}$ during the
simulation's final 20\%.
Green squares indicated those ringlets that settled into
a self-confining endstate having a higher libration amplitude, $10^{-3} < \Delta q \le 10^{-2}$, 
