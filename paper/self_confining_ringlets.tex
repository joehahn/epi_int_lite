...
Most proposed ringlet confinement mechanisms assume that there also exists one or more 
shepherd satellites whose gravitational perturbations keeps the ringlet radially confined, 
but the absence of such shepherds in Cassini observations of Saturn's rings
casts doubt upon this ringlet confinement mechanism.

The following uses a suite of N-body simulations to investigate whether ringlet
self-gravity can drive a narrow eccentric ringlet to evolve into a 
self--confining state. These simulations show that, under a wide variety of initial conditions,
an eccentric ringlet's secular perturbations of itself causes the eccentricity at it's outer
edge to grow at the expense of the eccentricity at it's inner edge. This causes the
ringlet's nonlinearity parameter $q$
to grow over time until it exceeds the $q\simeq\sqrt{3}/2$ threshold where 
the ringlet's orbit-averaged angular momentum flux due to ringlet viscosity + self-gravity is zero.
This absence of any net radial angular momentum transfer through the ringlet 
means that it has settled into a self-confining state, {\it i.e.} it does 
not spread radially due to its viscosity. Simulations also show that such ringlets have sharp edges.

Nonetheless, ringlet viscosity still circularizes the ringlet in time $\tau_e\sim10^6$ orbits $\sim1000$ years. 
After viscosity has reduced the eccentricity of the ringlet's 
inner edge to zero, the ringlet's nonlinearity parameter will then shrink over time,
angular momentum flux reversal will cease, and radial spreading will resume as ringlet 
viscosity again transmits an  outwards flux of angular momentum.

We then speculate that a sharp-edged self-confining narrow eccentric planetary ringlet first originated as
an exceptionally large ring particle orbiting within a nearby dense planetary ring. 
Possibly as a small embedded moonlet that is responsible for the propeller structures
...
cause that moonlet to migrate towards and then beyond the dense ring's edge. 
If so then a dense planetary ring could also birth multiple such migrating moonlets, 
and their differential migration rates will encourage moonlet-moonlet scatterings and eventual collision,
resulting in a narrow eccentric ringlet whose self-gravity could excite its eccentricity gradient
...
\citep{BGT86, SYL85}, whereas some might regard the execution of an epi\_int\_lite simulation 
...
routinely exhibit surface-brightness asymmetries as well other substructures that
...
birth rings can also be very long-lived. Which causes us to wonder whether the
