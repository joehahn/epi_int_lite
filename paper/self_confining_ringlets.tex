...
varies with moonlet mass, so a larger moonlet will eventually overtake a smaller moonlet that is orbiting exterior to it. 
Indeed, calculations by \cite{PS01} suggest that satellite Prometheus emerged from the outer edge of Saturn's A
ring $\sim10^7$ years ago, and that it will have a close encounter with Pandora in another $\sim10^7$ years.
...
of this least-speculative ringlet origin scenario would require estimating various timescales that are relevant here,
